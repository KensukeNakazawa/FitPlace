...
export default {
  ...
  created: function() {
    this.plan_exercise_id = this.$route.params['plan_exercise_id'];
    this.week_day_id = this.$route.query['week_day'];
    axios.get('/api/exercise_plans/exercises/' + this.plan_exercise_id)
    .then((res) => {
  ...
  methods: {
    ...
    update() {
      this.$store.dispatch('showLoading');
      axios.put('/api/exercise_plans/exercises/' + this.plan_exercise_id, {
